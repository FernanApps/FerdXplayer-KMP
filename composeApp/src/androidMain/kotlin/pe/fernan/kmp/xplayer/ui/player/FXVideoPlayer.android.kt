...
actual class FXVideoPlayer(
    ...
    private val listener = object : Player.Listener {
        override fun onPlaybackStateChanged(playbackState: Int) {
            when (playbackState) {
                Player.STATE_IDLE -> if (_status.value !is FXPlayerStatus.Error) _status.value = FXPlayerStatus.Idle
                Player.STATE_BUFFERING -> _status.value = FXPlayerStatus.Buffering
                Player.STATE_READY -> {
                    _status.value = FXPlayerStatus.Ready
                    emitDuration()
                }

                Player.STATE_ENDED -> _status.value = FXPlayerStatus.Ended
                else -> {}
            }
        }

        override fun onIsPlayingChanged(isPlaying: Boolean) {
            if (isPlaying) _status.value = FXPlayerStatus.Playing
            countingJob?.cancel()
            if (isPlaying) {
        ...
        override fun onPlayerError(error: PlaybackException) {
            _status.value = FXPlayerStatus.Error(error)
        }
    }
